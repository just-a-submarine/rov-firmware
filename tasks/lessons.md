...
### 文件互相矛盾時，別用「多數決」當真相——以實機/使用者為準
- **手把怎麼接手機**：doc/06、README、GS CONTEXT 三處都寫「藍牙連手機」，CLAUDE.md 寫「有線 USB」。
  我以為「三比一」就把 CLAUDE.md 改成藍牙——**結果反了**：使用者明說手把是**有線 USB 連手機**，CLAUDE.md 原本才對。
- 連帶查出第二個矛盾：doc/06 §一寫「不需 5V 升壓」，GS CONTEXT 卻寫「GS 仍用 5V/2A 升壓」且引用 doc/06 §一。
  真因＝**5V 升壓是給地面站板本身供電（doc/03 §一），不是給手把**；doc/06 把「沒手把→不需 5V」這因果寫錯。
- 教訓：① 文件數量不是證據權重，**矛盾時先找實機/使用者拍板**，別用多數決；② 改任何「現狀描述」前先 grep 全庫看有沒有反例，
  一次對齊所有出處（這次一改要掃 4 個檔），否則只會把矛盾從一處搬到另一處。
